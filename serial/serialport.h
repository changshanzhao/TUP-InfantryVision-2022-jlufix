...
typedef struct
{
    ...
    unsigned char isSwitched;   //目标是否发生切换
    unsigned char isFindTarget; //当识别的图片范围内有目标且电控发来的信号不为0x00（关闭视觉）置为1，否则置0
    unsigned char isSpinning;   //目标是否处于陀螺状态
    unsigned char ismiddle;     //设置1表示目标进入了可以开火的范围，设置0则表示目标尚未进入可开火的范围，默认置0
} VisionData;
